...
The mascot is the app icon's dog itself: `assets/mascot.png` is the icon artwork with its background keyed out, rendered by `MascotCallout`. Because it is the real artwork, the dog never changes; a state adds something around it (thought dots for `thinking`, confetti for `celebrating`) and the other states (`happy`, `encouraging`, `waiting`) exist so call sites name the moment. If the icon changes, regenerate `mascot.png` from it. It appears in exactly these places: the header of each tab root through `PageHeader` (with a speech bubble in its own voice, saying something true about that page), empty states, loading, and session completion. Profile is the exception: the dog's own avatar is already there, so the header has no mascot. It never appears twice on one screen, and never decorates a list, a card corner, or a button. Its speech-bubble lines are short, first-person plural ("We missed one. No big deal, let's pick it back up."), and never instruct the user to feel something.
