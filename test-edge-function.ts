...
const SUPABASE_URL = Deno.env.get("EXPO_PUBLIC_SUPABASE_URL") || 'https://bqckiotsddmrjnhsawce.supabase.co';
const SUPABASE_ANON_KEY = Deno.env.get("EXPO_PUBLIC_SUPABASE_ANON_KEY") || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJxY2tpb3RzZGRtcmpuaHNhd2NlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMzMjIxOTQsImV4cCI6MjA4ODg5ODE5NH0.wYSOrU21D6bahD65PYvA4SVAEQrwo6x5_rdi69_k2aY';
...
async function runTest() {
  ...
  console.log('User signed up/in:', user.id);
  ...
  if (fnError) {
    ...
    if (fnError.context) {
      console.error('Error Context:', await fnError.context.blob().then(b => b.text()));
    }
  } else {
